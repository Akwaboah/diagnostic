...
from I_CARE.models import Sale

def generate_report(request):
    # Query the Sale model for the data
    sales = Sale.objects.all().values('product', 'date', 'amount')

    # Convert the QuerySet to a Pandas DataFrame
    product_data = pd.DataFrame.from_records(sales, columns=['product', 'date', 'amount'])

    # Group the data by product and date and calculate the sum of amounts
    product_data = product_data.groupby(['product', 'date']).sum().reset_index()
    ...
    for i, product in enumerate(product_data['product'].unique()):
        product_subset = product_data[product_data['product'] == product]
        chart = workbook.add_chart({'type': 'line'})

        chart.add_series({
            'name': product,
            'categories': [product, 1, 0, len(product_subset), 0],
            'values': [product, 1, 2, len(product_subset), 2],
        })
        ...
        worksheet.write(0, i, product)
        worksheet.insert_chart(1, i, chart)
